...
pro bandpass_lineflux_NIRSPEC, file, blueside, redside, feature, ew=ew
	...
	spec=mrdfits(spectrum,1, /silent)
	...
	speccounts = spec.spec*1e-23;+median(speccounts)
	bluecontcounts = spec.spec[bluecontarr]*1e-23;+median(speccounts)
        redcontcounts = spec.spec[redcontarr]*1e-23;+median(speccounts)
        linecounts = spec.spec[linearr]*1e-23;+median(speccounts)
	...
	fullcounts = dcorrspec[fullarr]
	dcorrlineerr = 1/sqrt(ivarline)*1e-23;+1/sqrt(median(ivarline))*1e-23
	...
	;print, lun, mask, '   ', slit, '   ', totallineflux, '   ', errtotallineflux, '   ', linelum, '   ', errlinelum, '   ', normlinelum, '   ', errnormlinelum
; plotting stuff
	
	meanlam = mean(spec.lambda[linearr])
	print, 'The line flux of the line centered at ' + strcompress(string(meanlam),/remove_all) + ' Angstroms is ' + strcompress(string(totallineflux),/remove_all) + ' +/- ' + strcompress(string(errtotallineflux),/remove_all)
	
	if n_elements(EW) then begin

	;meanflam1 = (mean(bluecontcounts)*3e18/mean(blueband)^2 + mean(redcontcounts)*3e18/mean(redband)^2)/2
	meanflam = mean(cnt)*3e18/mean(lineband)^2
	
	;sigmeanflam1 = sqrt( (stddev(bluecontcounts)*3e18/mean(blueband)^2)^2 + (stddev(redcontcounts)*3e18/mean(redband)^2)^2) 
	sigmeanflam = sqrt(mean(errlinebackarr))
	;print, mean(errlinebackarr), mean(cnt)

	EWlimit = -(3*errtotallineflux)/(meanflam)
	
	
	EWlimiterr = EWlimit*(sigmeanflam)/meanflam
	print, 'The 3 sigma EW limit on the feature centered at ' + strcompress(string(meanlam),/remove_all) + ' Angstroms is ' + strcompress(string(EWlimit)) + ' +/- ' + strcompress(string(EWlimiterr)) + ' Angstroms'
	endif
	loadcolors
	splot, spec.lambda[fullarr], fullcounts, xrange=[blueside[0], redside[1]], yrange=[min(fullcounts)-median(fullcounts), max(fullcounts)+median(fullcounts)],psym=10
