...
;	delta lambda flux
function measure_bandpass_ew_func, $
	...
	goodpixB = where(spec.ivar[roiB] gt 1e-08)              ;changed 4/7 to remove fill gap areas where the ivar is set to 1e-30, could set this higher if wanted to exclude bad sky areas, CHANGE IF NOT USING DEIMOS DATA!!
        ...
        if goodpixF[0] ge 0 then ivarF = totivarF[goodpixF] else ivarF = totivarF
        ...
        if goodpixF[0] ge 0 then featurelam = totfeaturelam[goodpixF] else featurelam = totfeaturelam
        ...
        if goodpixF[0] ge 0 then featurespec = totfeaturespec[goodpixF] else featurespec = totfeaturespec
