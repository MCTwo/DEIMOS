...
function coadd_cluster_spectra, cat, normalize=normalize, zeroweight=zeroweight, $
   ...
   readcol, cat, format='F,F,A,A,A,F,A,F', RA, dec, obj, mask, slit, z, q, rband
...
;	for j=0, numread-1 do begin  
   ...
   list[i] = {maskname:mask[i], slitname:slit[i], objectno:obj[i], Iband:rband[i], absIflux:absIflux[i],  zquality:q[i], z:z[i]}
   endfor
   ...
   for i=0, nspec-1 do begin ;begin loop over input list
    ...
    spec = ('/Volumes/Data2/orelse/lemaux/deimos/N200/' + list[i].maskname + '/*/spec1d.' + list[i].maskname + '.' + list[i].slitname +'.' + list[i].objectno + '.fits')[0]
...
; now normalize 
   ...
   mwrfits, result, 'N200.068z070.coadd.blueresponse.fits'
   device, retain=2
