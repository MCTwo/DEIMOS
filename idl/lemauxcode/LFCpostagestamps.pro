pro LFCpostagestamps
	
	readcol, 'LAE_wtargets_all_RAdec_forLFC.dat', format='A,A,D,D,A,A,A', mask, slit, ra, dec, rfile, ifile, zfile, LAEra, LAEdec

	for i=0, n_elements(mask)-1 do begin

		print, mask[i] + '.' + slit[i]
		; getting slit PA and length from the bintabs file
		bintabs = '/Volumes/Data2/orelse/lemaux/deimos/sc1604/' + mask[i] + '/*/' + mask[i] + '.bintabs.fits'
		bintab = mrdfits(bintabs,3, /silent)
		index = where(strcompress(bintab.slitname, /remove_all) eq slit[i])
		
		;if index lt 0 then begin
	 	;	message, 'Couldnt find slit PA and length, check to make sure slit name is correct'	
		;endif
		
		slitlength = bintab[index].slitlen
		PA = bintab[index].slitlpa
		...
		sxaddpar, rhdr, 'CRVAL1', ra[i], ' R.A. (degrees) of reference pixel'
                ...
                sxaddpar, rhdr, 'slitPA', PA, 'Position angle of DEIMOS slit'
                sxaddpar, rhdr, 'slitlen', slitlength, 'Length (arcsec) of DEIMOS slit'
                sxaddpar, rhdr, 'object', mask[i]+slit[i], 'cutout DEIMOS object'	
		...
		;remember ORIENTA when using PA and making slits
                ...
                sxaddpar, ihdr, 'slitPA', PA, 'Position angle of DEIMOS slit'
                sxaddpar, ihdr, 'slitlen', slitlength, 'Length (arcsec) of DEIMOS slit'
                sxaddpar, ihdr, 'object', mask[i]+slit[i], 'cutout DEIMOS object'	
		...
		sxaddpar, zhdr, 'CRVAL1', ra[i], ' R.A. (degrees) of reference pixel'
                ...
                sxaddpar, zhdr, 'slitPA', PA, 'Position angle of DEIMOS slit'
                sxaddpar, zhdr, 'slitlen', slitlength, 'Length (arcsec) of DEIMOS slit'
                sxaddpar, zhdr, 'object', mask[i]+slit[i], 'cutout DEIMOS object'
