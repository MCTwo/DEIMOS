...
; nov 2008 bl, changed the nature of the input catalog, few other changes
;----------------------------------------------------------------------
...
function coadd_cluster_spectra, cat, outspec=outspec, normalize=normalize, zeroweight=zeroweight, $
                        ivartrim=ivartrim,inorm=inorm,dloglam=dloglam,weight=weight

   
   readcol, cat, format='A,A,A,A,D,D,D,D,D,D,A', obj, newobj, mask, slit, RA, dec, Rband, Iband, Zband, z, q
   numread = n_elements(RA)
   absIflux = fltarr(numread)   
   
   for i=0, numread-1 do begin
   if slit[i] lt 100 then slit[i] = '0' + slit[i] else slit[i]=slit[i]
   endfor
   
   ;readcol, '1322fullzqgte3.cat', format = 'A,F,F,A,A,F', objcat, RAcat, deccat, maskcat, slitcat, Ibandcat
   ;numcat = n_elements(objcat)

   ;for i=0, numcat-1 do begin

;	for j=0, numread-1 do begin  

	;if objcat[i] eq obj[j] then Iband[j] = Ibandcat[i]
   ...
   list[i] = {list, maskname:mask[i], slitname:slit[i], objectno:obj[i], Iband:iband[i], absIflux:absIflux[i],  zquality:q[i], z:z[i]}
   endfor
   ...
   for i=0, nspec-1 do begin ;begin loop over input list
    ...
    spec = ('/Volumes/Data2/orelse/lemaux/deimos/N5281/' + list[i].maskname + '/*/spec1d.' + list[i].maskname + '.' + list[i].slitname +'.' + list[i].objectno + '.fits')[0]
...
; some slosh in dates
    ...
    if strlen(spec) gt 1 then begin
     ...
     if inorm eq 0 then begin 
       ;ss1d=fill_gap_blue(spec,/tweak,/telluric,/silent,header=header) 
       ss1d=fill_gap(spec,/tweak,/telluric,/silent,header=header)
       wave=ss1d.lambda
       airtovac,wave
       ss1d.lambda=wave
     endif else ss1d=fill_gap(spec,/tweak,/silent,header=header)
     ;endif else ss1d=fill_gap_blue(spec,/tweak,/silent,header=header)
...
; now normalize 
   ...
   if n_elements(outspec) then outspec=outspec else outspec='youdidntnameit.narf.fits'
   mwrfits, result, outspec
   device, retain=2
