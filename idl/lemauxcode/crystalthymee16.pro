pro crystalthymee, cplot=cplot, makedat=makedat
	...
	for j=0, n_elements(arcnum0)-1 do begin
		num[j] = factor*(arcnum0[j] + arcnum1[j] +  arcnum2[j] + arcnum3[j] + arcnum4[j] + arcnum5[j] + arcnum6[j] + arcnum7[j] + arcnum8[j] + arcnum9[j] + arcnum10[j] + arcnum11[j] + arcnum12[j] + arcnum13[j] + arcnum14[j] + arcnum15[j] + arcnum16[j] + arcnum17[j] + arcnum18[j])
	endfor
	...
	device, decomposed=0
	set_plot, 'PS'	
	device, /color, xoffset=0.2, yoffset=0.2, xsize=8, ysize=8, /inches, filename='cl1604LAE_a16_MS.ps'
	...
	contour, num, loglstar, logphistar, /irregular, /cell_fill, levels=levels, c_colors=c_colors, xrange =[41.2, 43.2], charsize=2, xthick=6, ythick=6, charthick=5, thick=4, xstyle=1, yrange =[-4.45, -2.05], ystyle=1, xtitle=xtitle, ytitle=ytitle
        ...
        xyouts, 41.4, -4.2, textoidl('\alpha = -1.6'), charsize=2.2, charthick=4, color=0	
	device, /close_file
