...
function Standardize_Spectra, $
    ...
    d_lambdat = (1500.-900.)/18000. ; wavelength spacing in Angstroms of coadded spectrum
    lambdat = d_lambdat * dindgen(18000) + 900. ; wavelength array of coadded spectrum
    rspec = dblarr(n,18000) ; individual blueshifted and rebinned spectra [nspec, 12000]
    rivar = dblarr(n,18000) ; associated inverse variances [nspec, 12000]
    Lya_strength = dblarr(n)

    for i=0,n-1 do begin        ; for each individual spectrum
        ;smoothspecs = smooth(specs[i,*], 3)
        smooth_ivar, specs[i,*], ivars[i,*], 2.8/2.35, smoothspecs, smoothivars
        lambda = lambdas[i,*] / (1.+z[i])
...
function Coadd, $
    ...
    spect = dblarr(18000)       ; total flux
    ivart = dblarr(18000)       ; total inverse variance
...
pro Perform_Coadd
    ...
    ;numread = 14             ; number of columns to read

    ; read in the specifications for each serendip
    readcol, '1604LYA_q3_tarrem.dat',  format='A,I,D,I,A', mask, slit, z, q, file, skipline=2;, numline=numread
    numread = n_elements(file)
    ...
    finalspec = Coadd(rawLya, subset1) ; call the coaddition routine
    smooth_ivar, finalspec.spec, finalspec.ivar, sigma, spec, ivar
    finalspec.spec = spec
    finalspec.ivar = ivar
    mwrfits, finalspec, 'LYA_coadd_q3_tarrem.fits'
    device, retain=2
