...
function coadd_cluster_spectra, cat, normalize=normalize, zeroweight=zeroweight, $
   ...
   readcol, cat, format='F,F,A,A,A,F,A,F', RA, dec, obj, mask, slit, z, q, iband
...
;	for j=0, numread-1 do begin  
   ...
   list[i] = {maskname:mask[i], slitname:slit[i], objectno:obj[i], Iband:iband[i], absIflux:absIflux[i],  zquality:q[i], z:z[i]}
   endfor
...
; now normalize 
   ...
   mwrfits, result, 'N5281.080z084.coadd.redresponse.q2resolved.fits'
   device, retain=2
