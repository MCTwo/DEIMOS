...
function Standardize_Spectra, $
    ...
    d_lambdat = (5100.-3000.)/13000. ; wavelength spacing in Angstroms of coadded spectrum
    lambdat = d_lambdat * dindgen(13000) + 3000. ; wavelength array of coadded spectrum
    rspec = dblarr(n,13000) ; individual blueshifted and rebinned spectra [nspec, 12000]
    rivar = dblarr(n,13000) ; associated inverse variances [nspec, 12000]
    Lya_strength = dblarr(n)

    for i=0,n-1 do begin        ; for each individual spectrum
        ;smoothspecs = smooth(specs[i,*], 3)
        sig = (1.95)/(1+z[i])/1.17741
	;sig = 2
	smooth_ivar, specs[i,*], ivars[i,*], sig, smoothspecs, smoothivars
...
function Coadd, $
    ...
    spect = dblarr(13000)       ; total flux
    ivart = dblarr(13000)       ; total inverse variance
...
pro Perform_Coadd
    ...
    numread = 1             ; number of columns to read

    ; read in the specifications for each serendip
    readcol, 'aba.dat',  format='A,I,D,D,A,A', mask, slit, Iband, z, q, file, skipline=2, numline=numread
    ...
    finalspec.spec = spec
    finalspec.ivar = ivar
    mwrfits, finalspec, './newrestframe/spec1d.SC1NM1.086.rest.fits'
    device, retain=2
