...
function coadd_LYA_spectra, cat, outspec, normalize=normalize, zeroweight=zeroweight, $
                        ivartrim=ivartrim,inorm=inorm,dloglam=dloglam,weight=weight

   readcol, cat, format='A,A,F,F,A,A', mask, slit, z, q, file, skipline=2,  numline = numread
   numread = n_elements(file)   

   list = replicate({list}, numread)
   for i=0, numread-1 do begin
   
   list[i] = {list, maskname:mask[i], slitname:slit[i], z:z[i], zquality:q[i], file:file[i]}
   endfor
...
; now normalize 
    ...
    result = { spec: specsum, lambda: lambda, ivar: ivar, totn:totn}
      ...
      ;sig = 1.5 		;sigma in pixels of Gaussian smoothing kernal
      ;specout = fltarr(n_elements(specsum))
      ;ivarout = fltarr(n_elements(ivar)) 
      ;smooth_ivar, specsum, ivar, sig, specout, ivarout
      ;result2 = { spec: specout, lambda: lambda, ivar: ivarout, totn:totn} 
