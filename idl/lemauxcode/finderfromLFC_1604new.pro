pro finderfromLFC, setup, maketable, viewchart, makechart

	hdr = headfits('/Volumes/Data2/orelse/lemaux/LFC/sc1604/sc1604_full.fits')
	...
	readcol, '/Volumes/Data2/orelse/lemaux/deimos/sc1604/cluster_catalog/NIRSPEC/may08NIRSPEC/starcats/setup' + setup + '/all1604NIRSPECtargets.withACSpriorities.allpriorities.wACSmags.cat', format='A,A,A,A,D,D,D,D,D', tarID, tarpriority, mask, slit, RAtar, dectar, rbandtar, ibandtar, zbandtar	;get all target properties
	N = n_elements(RAtar)
	...
	readcol, '/Volumes/Data2/orelse/lemaux/deimos/sc1604/cluster_catalog/NIRSPEC/may08NIRSPEC/NIRSPECtargets.1604.forrealdPA.ACSpriorities.cat', format='A,A,A,A,D,D', compmask1, compmask2, tarpriority1, tarpriority2, tartard, tartarPA	;get all relative positions and PAs from potential target list
	...
	readcol, '/Volumes/Data2/orelse/lemaux/deimos/sc1604/cluster_catalog/NIRSPEC/apr08NIRSPEC/fullEW.spectroscopic.autocompile.blemaux.1604.clusteronly.zqgte3.cat', format='A,A,A', specID, specmask, specslit
	...
	readcol, '/Volumes/Data2/orelse/lemaux/deimos/sc1604/cluster_catalog/NIRSPEC/may08NIRSPEC/starcats/setup' + setup + '/' + mask[i] + '.' + slit[i] + '.staroutput.cat', format = 'A,A,D,D,D,D,D', refslitindv, LFCIDindv, dgsindv, PAgsindv, rbandgsindv, ibandgsindv, zbandgsindv
	...
	imagesize = 100.	;distance from center of image to vertical or horizonal edge in "
	...
	readcol, '/Volumes/Data2/orelse/lemaux/deimos/sc1604/cluster_catalog/NIRSPEC/may08NIRSPEC/starcats/setup'+ setup + '/goodguidestars.cat', format='A', gsID 	;get LFC IDs of good guide stars from list
	readcol, '/Volumes/Data2/orelse/lemaux/deimos/sc1604/cluster_catalog/finalcats/final.idradecmag.lfcpluscosmic.withsdss.cat', format= 'A,D,D,D,D,D', LFCcatID, LFCRA, LFCdec, LFCr, LFCi, LFCz 	;get RA/Dec of all LFC+COS objects from final LFC cat
	...
	for i=0, n_elements(gsID)-1 do begin	;get the RA/Dec of the good guide stars and then get PA/d relative to the 1st target (should change to both or arbitrary #) and mag info
	...
	for i=0, n_elements(gsID)-2 do begin ;calculate offsets of guide stars relative to one another
	...
	file = '/Volumes/Data2/orelse/lemaux/deimos/sc1604/cluster_catalog/NIRSPEC/may08NIRSPEC/starcats/setup' + setup + '/cutout.setup' + setup + '.fits' ;read in the finder chart
	finder = mrdfits(file,0)

	if n_elements(finder) lt 10 then begin ;if finder chart doesn't exist then make one

        mm = mrdfits('/Volumes/Data2/orelse/lemaux/LFC/sc1604/sc1604_full.fits')
	...
	openw, lun, '/Volumes/Data2/orelse/lemaux/deimos/sc1604/cluster_catalog/NIRSPEC/may08NIRSPEC/starcats/setup' + setup + '/description.setup' + setup + '.cat', /get_lun
	...
	for i=0, n_elements(gsID)-1 do begin			
	printf, lun, '16GS'+gsnum[i]+'NIR'+ setup, tab, goodgsID[i], tab, rank[i], tab, mask[0]+"."+slit[0], tab,  gsRAsexh[i], ':', gsRAsexm[i], ':', gsRAsexs[i], tab, gsdecsexdeg[i], ':', gsdecsexam[i], ':', gsdecsexas[i], tab, rbandgoodgs[i], tab, dgoodgs[i], tab, PAgoodgs[i], tab, dRAgoodgs[i], tab, ddecgoodgs[i], format='(a17, a1, a17, a1, i1, a1, a12, a1, i2.2, a1, i2.2, a1, d6.3, a1, i2.2, a1, i2.2, a1, d5.2, a1, d7.3, a1, d6.3, a1, d7.3, a1, d7.3, a1, d7.3)' 
	endfor

	if n_elements(PAgoodgs2) gt 1 then for i=0, 3-1 do begin
	printf, lun, '16GS'+gsnum[i]+'NIR'+ setup, tab, goodgsID[i], tab, rank[i], tab, mask[1]+"."+slit[1], tab, gsRAsexh[i], ':', gsRAsexm[i], ':', gsRAsexs[i], tab, gsdecsexdeg[i], ':', gsdecsexam[i], ':', gsdecsexas[i], tab, rbandgoodgs[i], tab, dgoodgs2[i], tab, PAgoodgs2[i], tab, dRAgoodgs2[i], tab, ddecgoodgs2[i], format='(a17, a1, a17, a1, i1, a1, a12, a1, i2.2, a1, i2.2, a1, d6.3, a1, i2.2, a1, i2.2, a1, d5.2, a1, d7.3, a1, d6.3, a1, d7.3, a1, d7.3, a1, d7.3)' 
	endfor	
	if n_elements(PAgoodgs3) gt 1 then for i=0, 3-1 do begin
        printf, lun, '16GS'+gsnum[i]+'NIR'+ setup, tab, goodgsID[i], tab, rank[i], tab, mask[2]+"."+slit[2], tab, gsRAsexh[i], ':', gsRAsexm[i], ':', gsRAsexs[i], tab, gsdecsexdeg[i], ':', gsdecsexam[i], ':', gsdecsexas[i], tab, rbandgoodgs[i], tab, dgoodgs3[i], tab, PAgoodgs3[i], tab, dRAgoodgs3[i], tab, ddecgoodgs3[i], format='(a17, a1, a17, a1, i1, a1, a12, a1, i2.2, a1, i2.2, a1, d6.3, a1, i2.2, a1, i2.2, a1, d5.2, a1, d7.3, a1, d6.3, a1, d7.3, a1, d7.3, a1, d7.3)' 
        endfor
	...
	printf, lun, gsnum[0] + '-' + gsnum[1], tab, '16GS'+gsnum[0]+'NIR'+ setup, tab, '16GS'+gsnum[1]+'NIR' + setup, tab, 3600*goodgsgsd[0], tab, 360/(2*!PI)*goodgsgsPA[0], tab, goodgsgsdRA[0], tab, goodgsgsddec[0], format='(a20, a1, a17, a1, a17, a1, d8.3, a1, d8.3, a1, d8.3, a1, d8.2)'	
	printf, lun, gsnum[0] + '-' + gsnum[2], tab, '16GS'+gsnum[0]+'NIR'+ setup, tab, '16GS'+gsnum[2]+'NIR'+ setup, tab, 3600*goodgsgsd[1], tab, 360/(2*!PI)*goodgsgsPA[1], tab, goodgsgsdRA[1], tab, goodgsgsddec[1], format='(a20, a1, a17, a1, a17, a1, d8.3, a1, d8.3, a1, d8.3, a1, d8.2)'
	printf, lun, gsnum[1] + '-' + gsnum[2], tab, '16GS'+gsnum[1]+'NIR'+ setup, tab, '16GS'+gsnum[2]+'NIR'+ setup, tab, 3600*goodgsgsd[2], tab, 360/(2*!PI)*goodgsgsPA[2], tab, goodgsgsdRA[2], tab, goodgsgsddec[2], format='(a20, a1, a17, a1, a17, a1, d8.3, a1, d8.3, a1, d8.3, a1, d8.2)'
	...
	window, /free, xsize=(imagepixsize*2), ysize=(imagepixsize*2)
	device, decomposed=1
	help, finder, /struct
	tv, finder	;display the finder chart

	for i=0, n_elements(gsID)-1 do begin	;for all good guide stars draw a circle of radius 2.7" at their location
	...
	tvbox, slitsize, (imagepixsize-1.), (imagepixsize-1.), color='0000FF'XL, angle=-tartarPAforslit[0]/(2*!PI)*360	;draw a slit at the center of the image with PA given by the PA calculated between targets
	device, decomposed=0
	endif
	...
	entry_device = !d.name
	set_plot, 'PS'
	device, /color, bits_per_pixel=24, filename = '/Volumes/Data2/orelse/lemaux/deimos/sc1604/cluster_catalog/NIRSPEC/may08NIRSPEC/starcats/setup' + setup + '/setup'+ setup+ '.finder.ps', xoffset=0.75, yoffset= 0.75, xsize=7.75, ysize=10.25, /inches
	...
	windowXSize = !d.x_size		; get the x-window size in device coordinates of the PS window
   	...
   	x0 = (0.75/7.5) * windowXSize	; starting x values is 0.5 in inches traqnsform to device coordinates
   	y0 = (4.25/10.0) * windowYSize
   	xsize = (6/7.5) * windowXSize	;the image is 5.75 inches inside the 6.5 inch window, transform to device coordinates
   	ysize = (5.65/10.0) * windowYSize
   	tv, bytscl(finder, min=-4300., max=-3570.), x0, y0, XSIZE=xsize, YSIZE=ysize	

	for i=0, 2 do begin     ;for all good guide stars draw a circle of radius 2.7" at their location

        tvcircle, 15.*xsize/(imagepixsize*2), gspix[i,0]*xsize/(imagepixsize*2)+x0, gspix[i,1]*ysize/(imagepixsize*2)+y0, color=6, THICK=3	;if I change the pixel size of the original cutout then 1000 needs to be changed in here

        endfor

        for i=0, N-1 do begin   ;for all targets draw a circle of radius 1.8" at their location

        tvcircle, 10.*xsize/(imagepixsize*2), tarpix[i,0.]*xsize/(imagepixsize*2)+x0, tarpix[i,1.]*ysize/(imagepixsize*2)+y0, color=5, THICK=2

        endfor

        slitsize = dblarr(2)    ;2 array with slit width and slit length 1" x 42"
        slitsize[0] = 1./0.18*xsize/(imagepixsize*2)
        slitsize[1] = 42./0.18*ysize/(imagepixsize*2)	;pixel scale is 0.18 in LFC unbinned differed if using a different image

        tvbox, slitsize, (imagepixsize-1.)*xsize/(imagepixsize*2)+x0, (imagepixsize-1.)*ysize/(imagepixsize*2)+y0, color=4, angle=-tartarPAforslit[0]/(2*!PI)*360, THICK=2	;this will also need to be changed if original image size is change 499 to new center

	for i=0, 2 do begin
	xyouts, gspix[i,0]*xsize/(imagepixsize*2)-x0/2, gspix[i,1]*ysize/(imagepixsize*2)+y0+300, rank[i], charsize=1.5, color=6, /device		;label the guide stars with their priority on the image
	endfor	

	for i=0, N-1 do begin
	xyouts, tarpix[i,0.]*xsize/(imagepixsize*2)+x0+300, tarpix[i,1.]*ysize/(imagepixsize*2)+y0, mask[i] + '.' + slit[i], charsize=1, color=5, /device 	;label the target names
	endfor

	linesize = dblarr(2)			;for creating N E legend on image
	linesize[0] = .1/0.18*xsize/(imagepixsize*2)
	linesize[1] = 15./0.18*ysize/(imagepixsize*2)	
	tvbox, linesize, 200.*xsize/(imagepixsize*2)+x0, 900.*ysize/(imagepixsize*2)+y0, angle=0, THICK=2, color=6
	tvbox, linesize, 200.*xsize/(imagepixsize*2)+x0-linesize[1]/2, 900.*ysize/(imagepixsize*2)+y0-linesize[1]/2, color=6, angle=90, THICK=3
	xyouts, 200.*xsize/(imagepixsize*2)+x0 - 150,  900.*ysize/(imagepixsize*2)+y0 + linesize[1]/2 + 200, 'N', charsize=1.25, color=6, /device
	xyouts, 200.*xsize/(imagepixsize*2)+x0 - linesize[1] - 300,  900.*ysize/(imagepixsize*2)+y0 - linesize[1]/2-150, 'E', charsize=1.25, color=6, /device
	...
	if n_elements(PAgoodgs2) gt 1 then for i=0, n_elements(gsID)-1 do begin
	xyouts, (0.01/7.0)*windowXsize, 6500-500*indexarr[N-1+3+i], goodgsID[i] + ' ' + string(rank[i], format='(I1)') + '  ' + mask[1]+'.'+slit[1] + '  ' + string(gsRAsexh[i], format='(D4.0)') +':0' + string(gsRAsexm[i], format='(D2.0)') + ':' + string(gsRAsexs[i], format='(D6.3)') + '  ' + string(gsdecsexdeg[i], format='(D4.0)') + ':0' + string(gsdecsexam[i], format='(D3.0)') + ':' + string(gsdecsexas[i], format='(D6.3)') + '  ' + string(rbandgoodgs[i], format='(D7.3)') + '  ' + string(dRAgoodgs2[i], format='(D7.3)') + '  ' + string(ddecgoodgs2[i], format='(D7.3)'), color=0, charsize=1,  /device	
	endfor
	...
	xyouts, (0.15/7.0)*windowXsize, 6500-500*[N-1+8], string(gsnum[0], format='(I1)') + '-' + string(gsnum[1], format='(I1)') + '       16GS'+ string(gsnum[0], format='(I1)') +'NIR'+ setup + '      16GS'+ string(gsnum[1], format='(I1)') +'NIR' + setup + '      ' +  string(goodgsgsdRA[0], format='(D8.3)') + '   ' +  string(goodgsgsddec[0], format='(D8.3)'), charsize=1, color=0, /device	
	xyouts, (0.15/7.0)*windowXsize, 6500-500*[N-1+9], string(gsnum[0], format='(I1)') + '-' + string(gsnum[2], format='(I1)') + '       16GS'+ string(gsnum[0], format='(I1)') +'NIR'+ setup + '      16GS'+ string(gsnum[2], format='(I1)') +'NIR' + setup + '      ' +  string(goodgsgsdRA[1], format='(D8.3)') + '   ' +  string(goodgsgsddec[1], format='(D8.3)'), charsize=1, color=0, /device
	xyouts, (0.15/7.0)*windowXsize, 6500-500*[N-1+10], string(gsnum[1], format='(I1)') + '-' + string(gsnum[2], format='(I1)') + '       16GS'+ string(gsnum[1], format='(I1)') +'NIR'+ setup + '      16GS'+ string(gsnum[2], format='(I1)') +'NIR' + setup + '      ' +  string(goodgsgsdRA[2], format='(D8.3)') + '   ' +  string(goodgsgsddec[2], format='(D8.3)'), charsize=1, color=0, /device
