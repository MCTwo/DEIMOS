...
function coadd_cluster_spectra, cat, outspec=outspec, normalize=normalize, zeroweight=zeroweight, $
   ...
   readcol, cat, format='A,A,A,A,D,D,D,D,D,D,A', obj, newobj, mask, slit, RA, dec, Rband, Iband, Zband, z, q;, numline=1, skipline=72
...
;	for j=0, numread-1 do begin  
   ...
   for i=0, nspec-1 do begin ;begin loop over input list
    ...
    spec = ('/Volumes/Data2/orelse/lemaux/deimos/0023/' + list[i].maskname + '/*/spec1d.' + list[i].maskname + '.' + list[i].slitname +'.' + list[i].objectno + '.fits')[0]
    print, spec
