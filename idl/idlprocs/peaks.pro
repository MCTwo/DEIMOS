...
function peaks,y,nsig,npk
on_error,2
d0 = y - shift(y,1)
d1 = y - shift(y,-1)
pk = where(d0 gt 0 and d1 gt 0,np)
if n_elements(nsig) gt 0 then begin
    yp = y[pk]
    mn = robust_mean(yp,4)
    sig = robust_sigma(yp)
    bigind = where(yp gt mn + nsig*sig,nb)
    if nb gt 0 then big = pk[bigind] else big = -1
endif else big = pk
if n_params() eq 3 then npk = n_elements(big)
