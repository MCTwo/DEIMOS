...
# 6. Convert to x,y in focal plane:
	rp = sqrt (xp*xp + yp*yp)
	foc = CAM_FOC(sys) + xp * XFO_TLT(sys) + yp * YFO_TLT(sys)
	theta = atan (rp * PIX_SZ / foc)
...
# double	cost, sint
# double	tanx, tany
double	foc				# effective focus (TMP -- here a ratio)
...
# Convert to x,y in focal plane:
	...
	xp = rp * cos (phi)
	yp = rp * sin (phi)
# ADDED -- kludgy and approx.
	foc = CAM_FOC(sys) + xp * XFO_TLT(sys) + yp * YFO_TLT(sys)
	rp = tan (theta) * foc / PIX_SZ
	xp = rp * cos (phi)
	yp = rp * sin (phi)
...
begin
	...
	xp = xics - bx
	yp = yics - by
	x =  cosa * xp + sina * yp + 1024.5
	y = -sina * xp + cosa * yp + 2048.5
