...
; the next two tables may not always exist!!!!
  dex = where(extnames eq 'ObjectCat', cnt1)
  if cnt1 eq 0 then message, 'No ObjectCat Table found!',/info $
     else objcat = mrdfits(bfile, dex[0], /silent)
  dex = where(extnames eq 'SlitObjMap', cnt2)
  if cnt2 eq 0 then message, 'No ObjectCat Table found!',/info $
     else objmap = mrdfits(bfile, dex[0], /silent)

  alltables = cnt1 ne 0 AND cnt2 ne 0
...
; now, the case where we don't have all the tables
...
if n_elements(mags) eq 0 then mags=fltarr(nslits)+99.
if n_elements(objnames) eq 0 then objnames=string(lindgen(nslits),format='(i7)')
