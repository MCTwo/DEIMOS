;+
; NAME:
;   deimos_mask_calibrate_newslider3
;
; PURPOSE:
;   top level routine to generate flats and arcs to a given mask,
;   with separate FITS files for each slitlet. For slider 3 data
;   taken after May of 2009
;
; CALLING SEQUENCE:
;   deimos_mask_calibrate_newslider3, planfile, chiplist=chiplist, $
...
pro deimos_mask_calibrate_newslider3,  planfile,  chiplist=chiplist, noplot=noplot, $
...
; process optical model for this chip
     ...
     model_lambda.lambda_y[0] = model_lambda.lambda_y[0] - 17.*(model_lambda.lambda_y[0] ne 0)          ;changed BL 6/24/09, suggested by Jeff Newman, trying these lines to change the optical model guess for the new FCS actuator, the actual central wavelength is about 26 A off of the reported central wavelength
     goodlammod = model_lambda.lambda_y[0,where(model_lambda.lambda_y[0] gt 0)]
     meangoodlammod = mean(goodlammod)
     
     model_lambda.lambda_y_top[0] = model_lambda.lambda_y_top[0] - 17.*(model_lambda.lambda_y_top[0] ne 0)
     model_lambda.lambda_y_bottom[0] = model_lambda.lambda_y_bottom[0] - 17.*(model_lambda.lambda_y_bottom[0] ne 0)
