...
;  bitmask = bitmask*1 OR (avg.mask NE 0)*128 OR (avg.mask EQ 2^nexp-1)*256
  ...
  ; NOTE -- next lines added by D.Stern to do extra sky subtraction,
  ;   (with 2nd+3rd change a little bit later) -- DS CHANGE
  if (chipno eq 1) or (chipno eq 4) or (chipno eq 5) or (chipno eq 8) then begin
	dsavgflux = dsbackground(avg.flux, orderscale=40)
  endif else begin
      dsavgflux = dsbackground(avg.flux, orderscale=64)
  endelse 


  if polyflag then begin
     ;result =  { flux:floatcompress(avg.flux), $ -- DS CHANGE
     result =  { flux:floatcompress(dsavgflux), $
