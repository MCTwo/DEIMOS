...
function  arc_merge_lambda, filelist, hdr, rebin=rebin, $
                             blue_end=blue_end, red_end=red_end, $
                             nonlocal=nonlocal
      ...
      if keyword_set(nonlocal) then begin
          ...
          if n_ext gt 2 then begin
            ss = mrdfits(filelist[i], 3, /silent) 
              ...
              ss.rawarc = (ss.rawarc * 0.)-1
          endelse
      endif else begin 
          ss = mrdfits(filelist[i], 1, /silent)
      endelse
; determine the number of rows in the ith file.
      nrows[i] = (size(ss.rawarc, /dimen))[1]
...
; check for NaN's in the slit data. set NaN's to be -1000.
      ind_NaN = where(finite(ss.rawarc) eq 0, cnt) 
      if cnt ne 0 then ss.rawarc(ind_NaN) = -1000. 
...
;        slit_aligned[indices[*,j],j] = ss.rawarc[*,j]
          lamrange=minmax(lambda2d[*,j])
          wh=where(lambda gt lamrange[0] and lambda lt lamrange[1],whct)
          if whct gt 0 then $
            slit_aligned[wh,j]=interpol(ss.rawarc[*,j],lambda2d[*,j],lambda[wh])
      endfor
