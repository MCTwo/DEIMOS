;+
; NAME:
;   deimos_mask_calibrate_newslider4
;
; PURPOSE:
;   top level routine to generate flats and arcs to a given mask,
;   with separate FITS files for each slitlet. For slider 4 data
;   taken after May of 2009.
;
; CALLING SEQUENCE:
;   deimos_mask_calibrate_newslider4, planfile, chiplist=chiplist, $
...
pro deimos_mask_calibrate_newslider4,  planfile,  chiplist=chiplist, noplot=noplot, $
...
; process optical model for this chip
     ...
     model_lambda.lambda_y[0] = model_lambda.lambda_y[0] + 8.		;changed BL 5/5/09, these three lines suggested by Jeff Newman to fix the slider four optical model problem
     model_lambda.lambda_y_top[0] = model_lambda.lambda_y_top[0] + 8.
     model_lambda.lambda_y_bottom[0] = model_lambda.lambda_y_bottom[0] + 33.
