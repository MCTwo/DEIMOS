;+
; NAME:
;   deimos_mask_calibrate_oldslider4
;
; PURPOSE:
...
; 
; CALLING SEQUENCE:
;   deimos_mask_calibrate_oldslider4, planfile, chiplist=chiplist, $
...
pro deimos_mask_calibrate_slider4_Marusa,  planfile,  chiplist=chiplist, noplot=noplot, $
     slitlist=slitlist, onlyalign=onlyalign,quick=quick, badarcs=badarcs, arcshift=arcshift
; added badarcs 7/7/14 BCL, see domask_oldslider4.pro
; added arcshift 7/8/14 BCL, see domask_oldslider4.pro

print, 'Arcshift is ' + strcompress(string(n_elements(arcshift)),/rem) 
...
; process optical model for this chip
     ...
     model_lambda.lambda_y[0] = model_lambda.lambda_y[0] + 75.			;changed BL 5/5/09, these three lines suggested by Jeff Newman to fix the slider four optical model problem
     model_lambda.lambda_y_top[0] = model_lambda.lambda_y_top[0]+ 75.		; changed 2/16/16, BCL for no good reason other than Marusa's data is a PIECE OF SHIT... or multiple pieces. Was 25/25/50
     model_lambda.lambda_y_bottom[0] = model_lambda.lambda_y_bottom[0] + 75.	; 75/75/75 seems to be working for most masks, not 212915B1 though, checking
     ;print, model_lambda.lambda_y[0]
...
; tracing: 7 sec
	...
	;print, xpos2[*,0]
	;help, xpos1, /struct
	;mwrfits, xpos1, 'xpos1.fits'
	;mwrfits, xpos2, 'xpos2.fits'
	;mwrfits, ypos1, 'ypos1.fits'
        ;mwrfits, ypos2, 'ypos2.fits'
	
	;openw, lun, 'slitcoords.cat', /get_lun
	;for i=0, n_elements(slitcoords)-1 do begin
	;	printf, lun, i
	;	printf, lun, slitcoords[i]
	;endfor
	;free_lun, lun
	;print, nslitlets
	;print, slitcoords, nslitlets
	;print, xpos1, xpos2, ypos1, ypos2
...
; COMBINE arcs, throwing out saturated lines only in 1 arc
    ...
    ;help, arcimage, /struct
    ;mwrfits, arcimage, 'arcimage.fits'
...
;   positions that are written to the spCalib files. 
    ...
    if finite(dxarc1) eq 0 or (chipno eq 4 and maskname eq 'M454D_2') then dxarc1=0	; ADDED 11/28/14 BCL, for when mask design fails, for Marusa's mask there should be no offset because I set the traces by hand kind of
...
; iterate once, just to be careful
    if quick le 0 then begin
       ...
       if finite(dxarc2) eq 0 or (chipno eq 4 and maskname eq 'M454D_2') then dxarc2=0        ; ADDED 11/28/14 BCL, for when mask design fails, setting offset between arcs and flats to zero
       dxarc = dxarc1+dxarc2
...
; Note that arcsat now carries vignetting info:
		...
		print, 'Badarcs is set to ' + string(n_elements(badarcs))
		if n_elements(badarcs) then begin
                deimos_fitwavelength, rect_arc, rect_arcsat+rect_nearvig, $
                 ...
                 dline=dline1, dlsig=dlsig1, wset=wset,sigma=sigma, badarcs=badarcs
		endif else begin
		deimos_fitwavelength, rect_arc, rect_arcsat+rect_nearvig, $
                 rect_arcivar*(rect_nearvig EQ 0B), lamps, chipno, $
                 grating, lambda_c, slitcoords[indblu[slitno]], $
		...
		if n_elements(badarcs) then begin
                 print, 'Badarcs is set to ' + string(n_elements(badarcs)) + ' and it looks like your grating or lambda_c is wrong'
...
;               message, 'No wavelength solution, skipping', /info
	       ...
	       ;vprint,4, 'The failure was either n_elements(wave) = ' + strcompress(string(n_elements(wave)),/rem) + ' or n_elements(polyx) = ' + strcompress(string(n_elements(polyx)),/rem) + ' or sigma = ' + strcompress(string(sigma),/rem)
...
;else begin
		if n_elements(arcshift) then begin	; Added BCL 7/8/14
			if arcshift[0] eq 1 then print, 'WARNING: Arcshift set to 1, you may have defaulted the value, put in a number!'
			print, 'Applying arcshift, wavelength solution of calibration frames shifted by ' + strcompress(string(arcshift),/rem) + ' Angstroms'
			polyx[0] = polyx[0]+arcshift[0]	; need to tweak the wavelength solution 12A to the red
							; for some masks cuz the arc frame is offset w.r.t. the	
							; science frame. Only the 0th order coefficient needs
							; to be changed
		endif
		;print, 'Narf2' 
                ;print, polyx 
