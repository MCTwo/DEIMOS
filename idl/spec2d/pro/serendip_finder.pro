pro serendip_finder, field, mask, slit, offset, multiband=multiband, band=band, fieldnum=fieldnum

; Created BCL 12/9/09
;
; Modified 12/6/10 to fix a major error where I was not accounting for the slit PA (since it was normally 0 in earlier fields) 
;
...
; if multiband is set then will output static plot windows of all three bands in addition to the atv window 
	...
	objpos = objposas/0.18 -  slitlen/2./0.18		;target trace position from middle of slit 

	print, 'The slit length is ' + strcompress(string(slitlen), /remove_all) + '" and the target object is ' +  strcompress(string(objpos*0.18), /remove_all) + '" from the middle of the slit'

	; now read in the LFC image corresponding to the field of interest

	if field ne 'XL005' and field ne 'XLSS005' then begin
		filecount = findfile('/Volumes/Data2/orelse/lemaux/LFC/*' + field + '*/*' + field + '*r_shift_norm.fits', count=numfields)
	
		if numfields eq 1 then begin
	
			fileR = '/Volumes/Data2/orelse/lemaux/LFC/*' + field + '*/*' + field + '*r_shift_norm.fits'
			fileI = '/Volumes/Data2/orelse/lemaux/LFC/*' + field + '*/*' + field + '*i_shift_norm.fits' 
			fileZ = '/Volumes/Data2/orelse/lemaux/LFC/*' + field + '*/*' + field + '*z_shift_norm.fits'
			...
			fileR = '/Volumes/Data2/orelse/lemaux/LFC/*' + field + '*/*' + field + '_' + fieldnum + '_r_shift_norm.fits'
                	fileI = '/Volumes/Data2/orelse/lemaux/LFC/*' + field + '*/*' + field + '_' + fieldnum + '_i_shift_norm.fits'
                	fileZ = '/Volumes/Data2/orelse/lemaux/LFC/*' + field + '*/*' + field + '_' + fieldnum + '_z_shift_norm.fits'
	...
	if field ne 'XL005' then readcol, '/Volumes/Data2/orelse/lemaux/LFC/phot_cats/lfc.*' + field + '*.cat', format='A,D,D,D,D,D', LFCID, LFCRA, LFCDec, LFCrband, LFCiband, LFCzband, /silent
	...
	closeobj = where(LFCRA gt RAtar - slitlen/3600. - 3./3600. and LFCRA lt RAtar + slitlen/3600. + 3./3600. and LFCDec gt Dectar-slitlen/3600. - 3./3600. and LFCDec lt Dectar+slitlen/3600. + 3./3600. and LFCID ne strcompress(IDtar,/remove_all))
	...
	slitsize = dblarr(2)    ;2 array with slit width and slit length taken from the info in the bintabs file 
        slitsize[0] = slitwidth/0.18
        slitsize[1] = slitlen/0.18
	...
	sercenter = [imagepixsize - offsetas/0.18*sin(slitPA*2.*!pi/360.), imagepixsize + offsetas/0.18*cos(slitPA*2.*!pi/360.)]
	...
	if closeobj[0] ne -1 then begin	
		dtarLFCRA = -1.*dtarLFC*sin(ptarLFC)/0.18		;get the offset from the target to the closeby objects from the previous calculation, both RA & dec output in pixels, RA is negative because of of the orientation 
	dtarLFCdec = dtarLFC*cos(ptarLFC)/0.18	
