...
;
; NAME
;      fill_gap_reallyred.pro
;
; PURPOSE
;      Stitches together the red and blue portions of an object
;      spectrum. 
;      *USE WHEN THE CENTRAL WAVELENGTH OF THE SETUP IS 7900 A OR MORE* 
;
; SYNTAX
...
function fill_gap_reallyred, file1d, nbuf=nbuf, npoly=npoly, nbins=nbins, $
...
; remove the a-band atmospheric absorption.
  if telluric then begin
      ...
      fix_response_reallyred,ss1
      if extnum gt 1 then begin 
          remove_telluric, ss2, airmass,silent=silent
	  fix_response_reallyred,ss2
      endif
  endif
...
; lastly, fill the gap between the red and blue spectra. 
	  ...
	  blu_lev = median(ss1.spec[nlam-1:nlam-50])    ;changed 6/2/2008, was derived from a single array value at the end of the blue chip spectrum
