...
pro reduce1d, cresult, files=files, doplot=doplot, debug=debug, $
              allresult=allresult, today=today, nlsky=nlsky, blue=blue, red=red
...
; fill the gap between the blue and red portions of the 1-d spectra.
	...
	if n_elements(blue) gt 0 then begin					;CHANGED BL 6/24/09 to make the choice of the blue/red fill_gap automatic, added blue keyword
		print, 'Warning: Using blue response curve and fill_gap, central wavelength should be less than 7300 A'
		if nlsky then ss1d = fill_gap_blue(files[ii], /nlsky, boxsprof=boxsprof,/tweak) $
     		else ss1d = fill_gap_blue(files[ii], boxsprof=boxsprof,/tweak)
	endif else begin			
			
		if n_elements(red) gt 0 then begin
			print, 'Warning: Using reallyred response curve and fill_gap, central wavelength should be greater than 7900 A'
        	        if nlsky then ss1d = fill_gap_reallyred(files[ii], /nlsky, boxsprof=boxsprof,/tweak) $
	                else ss1d = fill_gap_blue(files[ii], boxsprof=boxsprof,/tweak)
		endif else begin

		print, 'Warning: Using standard red response curve and fill_gap, central wavelength should be between 7300 A and 7900 A'
