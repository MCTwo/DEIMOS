...
pro reduce1d_m31, cresult, files=files, doplot=doplot, debug=debug, $
                  allresult=allresult, today=today, nlsky=nlsky

; put into the spec1d log stderr file the location of the spec1d
; routines.
  findpro, 'reduce1d_m31', dirlist=directories, /NOPRINT
...
;  eigenfile_star = 'spEigenStarM312.fits'
...
; fill the gap between the blue and red portions of the 1-d spectra.
      ...
      if nlsky then ss1d = fill_gap(files[ii], /nlsky, boxsprof=boxsprof) $
      else ss1d = fill_gap(files[ii], boxsprof=boxsprof)
...
; first set the parameters for call to zfind.
          ...
          nfind = 3
          npoly = 0             ; 3
          columns = [0,  1]     ;include absorption and emission templates in all cases
...
;              ' ZMIN=', zmin, ' ZMAX=', zmax, ' PSPACE=', pspace
          res_star = zfind_star_m31(ss1d, /linear_lambda, $
          ...
          result = [result, res_star[0:5]] ; Append result of top 2 star
...
;          zmax = max(ss1d.lambda)/1215. -1  ; Max range we can expect to see
          zmax = 5.
          pspace = 10
          nfind = 1             ;find best QSO candidate z
