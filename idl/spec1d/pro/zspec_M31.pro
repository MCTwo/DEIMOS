...
FUNCTION zspec_1d_read, nonlocal
; Read in the 1d spectrum

  COMMON results_COMMON, results, slitcount, q_index
  COMMON directory_COMMON, fulldir
  COMMON twod_data_COMMON, fluxdata, extraction_r1, extraction_r2, extraction_offset
...
; -------- read in data with fill_gap
  IF(nonlocal EQ 1) THEN $
    data = fill_gap(specname[0], /nlsky, /silent, _extra=extra) $
  ELSE $
    data = fill_gap(specname[0], /silent, _extra=extra);,  optimal = (boxcar eq 0))
...
;  eigen_name1 = concat_dir(deep_dir, 'templates/spEigenStarDeep2.fits')
  eigen_name1 = concat_dir(deep_dir, 'templates/spEigenStarM312.fits')
...
; median smooth the absorption-lien template spectrum.
  ...
  CASE state.smooth_on OF
    '0':  BEGIN
       splot, lambdas, specdata.spec, $
         ...
         ytitle='Counts',  yrange=[mins_new,maxs]
...
FUNCTION zspec_2d_read, spec1d, nonlocal
...
PRO zspec_showtext, state
  ...
  string10 = '   vel. dispn  = ' + string(zres.vdisp)
...
PRO zspec_update_atv, state
  ...
  nbins = (size(fluxdata.blambda, /dimensions))[0]
  nbins = nbins/4
  height = n_elements(fluxdata.bflux[0,*])+2 ; 2 for the two rows of lam info
  blambda0 = fluxdata.blambda[*,0]
  rlambda0 = fluxdata.rlambda[*,0]

  ; Split spectrum into 8 segments and then recombine
  flux1 = fluxdata.bflux[0:nbins-1,*]
  flux2 = fluxdata.bflux[nbins:2*nbins-1,*]
  flux3 = fluxdata.bflux[2*nbins:3*nbins-1,*]
  flux4 = fluxdata.bflux[3*nbins:4*nbins-1,*]
  ...
  flux1 = fluxdata.rflux[0:nbins-1,*]
  flux2 = fluxdata.rflux[nbins:2*nbins-1,*]
  flux3 = fluxdata.rflux[2*nbins:3*nbins-1,*]
  flux4 = fluxdata.rflux[3*nbins:4*nbins-1,*]
  ...
  atv, [[flux_red],[flux_blue]], min=-5, max=25, _extra=extra


  hlambda = (size(fluxdata.blambda))[2] < (size(fluxdata.rlambda))[2]
  lambdabottom = [blambda0, rlambda0]
  lambdatop = [fluxdata.blambda[*,hlambda-1], fluxdata.rlambda[*,hlambda-1]]
  offset = (height - (size(fluxdata.blambda, /dimensions))[1])/2>0
  ...
  if extraction_offset gt 0 then offset = -extraction_offset $
  else offset = offset-1


  for i=0, 7 do begin
  	base = fltarr(10) + height*i + offset
  	; left side
...
; template.
  ...
  values[6] = string('(*) NeIII', format='(a10)') + string(linewaves[6])
  values[7] = string('(*) HeI', format='(a10)') + string(linewaves[7])
  values[10] = string('(*) He', format='(a10)') + string(linewaves[10])
  values[11] = string('(*) Hd', format='(a10)') + string(linewaves[11])
  values[13] = string('(*) Hg', format='(a10)') + string(linewaves[13])
  ...
  tmps = ['Galaxy', 'Abs-Line', 'Emi-Line', 'Star', 'QSO']
  tmpdex = 0
...
; -1.0.
      if tmpdex eq 1 then zres_new.theta[1] = -1.0
      if tmpdex eq 2 then zres_new.theta[0] = -1.0
  endif ELSE BEGIN
    print, 'you disagreed...'
    return
  ENDELSE
...
; view menu.
  ...
  IF(state.qonly EQ 1) THEN BEGIN
    IF(slicount = -1) THEN slitcount=q_index[0] $
    ELSE BEGIN
...
PRO zspec, mname, local_only=local_only, nodeep=nodeep, _extra=extra
; Main routine
...
; check local_only and notdeep keywords.
  ...
  linenames = ['Lya', 'CIV', 'MGII', 'OII', 'OII', $
               'OII', 'NeIII', 'HeI', 'CaK', 'CaH', $
               'He', 'Hd', 'G Band', 'Hg',  'Hb', $
               'OIII', 'OIII', 'Mgb', 'NaD', 'NII', $
               'Ha', 'NII', 'SII', 'SII', 'Ca', $
               'Ca', 'Ca']
...
; displayed in the "Fit for z" widget.
  linewaves = [1215.67, 1549.5, 2799.11, 3727.55, 3726.03, $
               3728.82, 3869., 3889., 3933.7, 3968.5, $
               3970.07, 4101.73, 4303.6, 4340.5, 4861.3, $
               4958.9, 5006.9, 5173., 5893.0, 6548.1, $
  ...
  ;-----------------------------------
  if nodeep eq 1 then maskname = strcompress(mname,  /rem) $
...
; check for each slit if there is more than 1 object in the slit.
  ...
  savename = 'zspec.' + getenv('LOGNAME') + '.' +  maskname + '.fits'    ; name of the output file
  dumpname = 'zspec.' + getenv('LOGNAME') + '.' + maskname + '.sav'    ; name of the output file
