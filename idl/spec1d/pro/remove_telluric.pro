...
pro remove_telluric, spec1d, airmass, plot = plot,silent=silent

if n_elements(silent) eq 0 then silent=0


   common comm_telluric, aaa,  aspl, aband_airmass, $
                    bbb,  bspl, bband_airmass ;band data
...
; check that A band falls on the 1-d spectrum...
   ...
   if acnt eq 0 then begin
       if not(silent) then print, 'A-band not in spectral window...' 
   endif else begin 
...
; cross-correlate to find shift between template and input spectrum
;   get_shift, lambda[almin:almax], flux[almin:almax],
;   lambda[almin:almax], aband_flux,  pixel_shift
           pixel_shift=0.

;           angstrom_shift = pixel_shift*0.325
   ; this is the shift of the input spectrum relative to the template
; THE WAVELENGTHS IN CHUCK'S TEMPLATE ARE WRONG!!!
;           angstrom_shift = -0.77
   ...
   ; re-trim the input spectrum to just cover the shifted A band
           a_lambda_low = where(lambda lt aa.a_lambda[0], lowcnt) 
           if lowcnt eq 0 then almin = 0 $
           else almin = min([max(a_lambda_low),n_elements(lambda)-2])+1
           a_lambdamax = where(lambda gt aa.a_lambda[npix-1], upcnt)
           if upcnt eq 0 then almax = npixsp-1 $
           else almax = min(a_lambdamax)-1
...
; check that B band falls on the 1-d spectrum...
   ...
   if bcnt eq 0 then begin
       if not(silent) then print, 'B-band not in spectral window...' 
    endif else begin 
