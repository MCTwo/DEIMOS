...
; Read in the 1d spectrum
  ...
  specfile = 'spec1d.'+results[slitcount].maskname+'.'+results[slitcount].slitname+'.serendip1.fits'
...
PRO zspec_showtext, state
  ...
  string10 = '   A star = ' + string(format = '(2f8.2)', zres.theta[2])
;  string11 = '   vel. dispn  = ' + string(zres.vdisp)
...
PRO zspec_update_atv, state
  ...
  atv, [[flux_red],[flux_blue]], _extra=extra, /stretch
...
PRO zspec_ser1, mname, local_only=local_only, nodeep=nodeep, _extra=extra
; Main routine
...
;  print,zbest
  ...
  FOR i=0, nslit-1 DO BEGIN
    IF(strmid(objlist[i],0,9) EQ 'serendip1') THEN $
      serendip_count = serendip_count + 1
  ENDFOR
  print, 'No of serendips: ', serendip_count

;  print, objlist
; keep original number of slits, we need this later
  nslit_orig = nslit
  serendip = 0
  objnames = strarr(serendip_count)
  slnames  = strarr(serendip_count)
  ;print, "foo"

  FOR i=0,nslit-1 DO BEGIN
    IF(STRMID(objlist[i],0,9) EQ 'serendip1') THEN BEGIN
...
;  print, nslit, objnames
;  print, slitnames
; At this point we slitnames and objnames have ONLY the serendips so
; we don't need to deal with sky etc.
  ...
  IF(ct EQ 1) THEN BEGIN
    ...
    FOR i=0,  nslit-1 DO BEGIN
;        print, objnames[i],slitnames[i]
;        print, info.objno, info.slitno
      cnt = WHERE(info.objno EQ objnames[i] AND info.slitno EQ slitnames[i], c)
      IF c NE 2 THEN PRINT, 'Wrong number of entries in infofile for obj: ',  objnames[i],slitnames[i]
      IF c EQ 0 THEN BEGIN
          print, 'Error:  No entry in obj_info for: ', objnames[i], slitnames[i], '!!!'
...
;  print, zbest.objname, zbest.slitname
  FOR i=0, nslit-1 DO BEGIN
...
;    else results[i].zresult = zbest[dex[0]] 
    results[i].zresult = zbest[dex[0]] 
;    results[i].zresult  = zbest[i]
;    print, results[i].zresult, results[i].slitname, results[i].objname
;    print, zall.objname, zall.slitname
     cnt = where(strtrim(zall.objname) eq strtrim(objnames[i]), ct)
;     print, cnt[i*10:i*10+9]
;     print, zall[60:69]

; Now the trick with serendips is that they all have the same names
; so ct=nslit*10 instead of just 10 and so is cnt
; If we believe that we always have 10 entries for each object in zall
; we can say that cnt for each object is just i*10

    results[i].zresult_all=zall[cnt[i*10:i*10+9]]
...
;    ENDELSE
;    results[i].zresult_all[0:9] = zall[cnt]
;    print, results[i].zresult_all
  ...
  ENDIF ELSE $
      results[i].zresult_non = zall[cnt[i*10:i*10+9]]
  ENDFOR
...
; define the date to be used in the output file.
  ...
  savename = 'zspec.' + getenv('LOGNAME') + '.' +  $
    maskname + '.' + savdate + '.ser1.fits'    ; name of the output file
  dumpname = 'zspec.' + getenv('LOGNAME') + '.' + $
    maskname + '.' + savdate + '.ser1.sav'           ; name of the output file
